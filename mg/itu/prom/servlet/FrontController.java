...
public class FrontController extends HttpServlet {
    ...
    protected void setBackPage(HttpServletRequest request,HttpServletResponse response,  BindingResult rs, Method method) throws Exception
    {
        if (method.isAnnotationPresent(ErrorHandlerUrl.class)) {
            ...
            Mapping mapping =  this.methods.get(handlerUrlPage);
            if(mapping == null){
                // PrintWriter out = 
                throw new Exception("404 not found: l'url " + handlerUrlPage + " n'existe pas.");
            }
            Object value = MethodParameters.invokeMethod(config, mapping, request,response, verb);
